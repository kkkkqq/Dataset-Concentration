...
from generate import CodeReader, generate_images
def main_proc(rank, world_size, args):
    ...
    seed = args.seed if args.seed is not None else config.default_seed(args.num_pivots, True)
    if seed is None:
    ...
    logger('reading encoded samples...')
    template_codes_lst = CodeReader(args.feat_path, class_tags)
    logger('done\n\n')

    logger(f'generating {args.num_pivots} samples...')
    imgs_lst = generate_images(device, args.dit_path, dit_labels, template_codes_lst, args.num_pivots, args.no_opt, args.sort_strength, image_size=args.image_size, dit_bs = args.dit_bs, vae_bs = args.vae_bs, use_tqdm=(rank==0 if args.tqdm else False), latin=not args.no_latin, seed=seed, reproduce=not args.random)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vae-bs", type=int, default=25)
    parser.add_argument("--shift", type=int, default=0)
    parser.add_argument("--sort-strength", type=float, default=0.0005)
